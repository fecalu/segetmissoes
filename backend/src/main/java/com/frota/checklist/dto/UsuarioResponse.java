...
public record UsuarioResponse(Long id, String nome, String login, String cpf, Perfil perfil, boolean acessoHabilitado) {
    public static UsuarioResponse from(Motorista usuario) {
        return new UsuarioResponse(usuario.getId(), usuario.getNome(), usuario.getLogin(), usuario.getCpf(),
                usuario.getPerfil(), usuario.isAcessoHabilitado());
    }
}
